...
class Trader:
    ...
    def coconuts_coconut_coupon_strategy(self, state: TradingState):
        ...
        self.coconut_df = pd.concat([self.coconut_df, new_entry], ignore_index=True)
        print("Updated coconut_df:\n", self.coconut_df.tail())
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], Any, Any]:
        ...
        # COCONUT STRAT
        try:
            result[COCONUT] = self.coconuts_coconut_coupon_strategy(state)
        except Exception as e:
