...
class Trader:
    ...
    def __init__(self) -> None:
        ...
        # self.past_prices = {
        #     product: pd.DataFrame(columns=["Price"]) for product in PRODUCTS_COCONUT
        # }
        ...
        # Add MTSI and signal to the past prices data structure
        for product in PRODUCTS_COCONUT:
            self.past_prices[product] = pd.DataFrame(
                columns=["Price", "MTSI", "Signal", "Buy", "Sell"]
            )

        self.coconut_df = pd.DataFrame(
            columns=[
                "Price",
                "PC",
                "SPC",
    ...
    # def update_mtsi(self, product, price):
    #     """
    #     Updates the MTSI values for the given product using the latest price.
    #     """
    #     # Append the latest price directly to the DataFrame using .loc
    #     new_index = len(self.past_prices[product])
    #     print(f"____new_index:{new_index}_____")
    #     self.past_prices[product].loc[new_index] = {
    #         "Price": price,
    #         "PC": None,
    #         "SPC": None,
    #         "DSPC": None,
    ...
    #         "Sell": None,
    #     }
    #     print(f"____self.past_prices:{self.past_prices}_____")

    #     df = self.past_prices[product]

    #     print(f"____ df:{df}_____")

    #     # Ensure there are enough data points to calculate MTSI
    #     if len(df) > self.mtsi_params[product][1] * 2:
    #         print(f"____ self.mtsi_params:{self.mtsi_params}_____")
    #         # Calculate the MTSI components
    #         df["PC"] = df["Price"].diff()
    #         df["SPC"] = (
    #             df["PC"].ewm(span=self.mtsi_params[product][0], adjust=False).mean()
    #         )
    #         df["DSPC"] = (
    #             df["SPC"].ewm(span=self.mtsi_params[product][1], adjust=False).mean()
    #         )
    #         df["APC"] = abs(df["PC"])
    #         df["SAPC"] = (
    #             df["APC"].ewm(span=self.mtsi_params[product][0], adjust=False).mean()
    #         )
    #         df["DSAPC"] = (
    #             df["SAPC"].ewm(span=self.mtsi_params[product][1], adjust=False).mean()
    #         )
    #         df["TSI"] = 100 * df["DSPC"] / df["DSAPC"]
    #         df["MTSI"] = (
    #             df["TSI"].ewm(span=self.mtsi_params[product][2], adjust=False).mean()
    #         )
    #         df["Signal"] = (
    #             df["MTSI"].rolling(window=self.mtsi_params[product][2]).mean()
    #         )
    #         df["Buy"] = (df["MTSI"] > df["Signal"]) and (
    #             df["MTSI"].shift(1) <= df["Signal"].shift(1)
    #         )
    #         df["Sell"] = (df["MTSI"] < df["Signal"]) and (
    #             df["MTSI"].shift(1) >= df["Signal"].shift(1)
    #         )

    #         # Extract the latest signals and return them
    #         latest_signals = df.iloc[-1]
    #         return latest_signals["Buy"], latest_signals["Sell"]

    # def coconuts_coconut_coupon_strategy(self, state: TradingState):
    #     orders = []
    #     orders_coconuts = []
    #     orders_coconut_coupon = []

    #     # Get the mid prices for COCONUT and COCONUT_COUPON
    #     mid_price_coconuts = self.get_mid_price(COCONUT, state)
    #     mid_price_coconut_coupon = self.get_mid_price(COCONUT_COUPON, state)
    #     spread = mid_price_coconuts - mid_price_coconut_coupon

    #     print(f"| mid_price_coconuts: {mid_price_coconuts} |")
    #     print(f"| mid_price_coconut_coupon: {mid_price_coconut_coupon} |")
    #     print(f"| spread: {spread} |")

    #     # Update MTSI and retrieve the latest signals
    #     buy_signal, sell_signal = self.update_mtsi(COCONUT, mid_price_coconuts)
    ...
    #     # Calculate the position you have for COCONUT and COCONUT_COUPON
    #     position_coconuts = self.get_position(COCONUT, state)
    #     # position_coconut_coupon = self.get_position(COCONUT_COUPON, state)

    #     # Based on signals, decide whether to buy or sell COCONUT
    #     if buy_signal and position_coconuts < self.position_limit[COCONUT]:
    #         volume_to_buy = self.position_limit[COCONUT] - position_coconuts
    #         orders.append(Order(COCONUT, mid_price_coconuts, volume_to_buy))

    #     if sell_signal and position_coconuts > -self.position_limit[COCONUT]:
    #         volume_to_sell = self.position_limit[COCONUT] - position_coconuts
    #         orders.append(Order(COCONUT, mid_price_coconuts, -volume_to_sell))

    #     # index = len(self.past_prices[COCONUT])
    #     # self.past_prices[COCONUT].loc[index] = {
    #     #     "Price": mid_price_coconuts
    #     # }  # Fixed line
    #     # self.update_mtsi(COCONUT, mid_price_coconuts)

    #     # # Update past_prices and calculate MTSI for COCONUT_COUPON
    #     # index = len(self.past_prices[COCONUT_COUPON])
    #     # self.past_prices[COCONUT_COUPON].loc[index] = {
    #     #     "Price": mid_price_coconut_coupon
    #     # }  # Fixed line
    #     # self.update_mtsi(COCONUT_COUPON, mid_price_coconut_coupon)

    #     # coconut_best_ask, _ = next(
    #     #     iter(state.order_depths[COCONUT].sell_orders.items()), (None, None)
    #     # )
    #     # coconut_best_bid, _ = next(
    #     #     iter(state.order_depths[COCONUT].buy_orders.items()), (None, None)
    #     # )
    #     # print(f"....> coconut_best_ask:  {coconut_best_ask}")
    #     # print(f"....> coconut_best_bid:  {coconut_best_bid}")

    #     return orders

    def coconut_mtsi_strategy(self, state: TradingState):
        product = COCONUT
        mid_price_coconut = self.get_mid_price(product, state)

        # Append the mid price value to the Price column in the dataframe
        new_index = len(self.coconut_df)

        self.coconut_df.loc[new_index] = {
            "Price": mid_price_coconut,
            "PC": None,
            "SPC": None,
            "DSPC": None,
            "APC": None,
        ...
        print(f"___ coconut_df {len(self.coconut_df)}")
        # print(f"___ coconut_df {self.coconut_df}")

        # Check if the length of the dataframe is greater than mtsi_params[product][1] * 2
        # print(f"___ mtsi_params {self.mtsi_params['COCONUT'][1]}")

        r, s, signal_ma = 25, 10, 10

        if len(self.coconut_df) > s * 2:
            df = self.coconut_df
            ...
            df["APC"] = abs(df["PC"])
            df["SAPC"] = df["APC"].ewm(span=r, adjust=False).mean()
            df["DSAPC"] = df["SAPC"].ewm(span=s, adjust=False).mean()
            df["TSI"] = 100 * df["DSPC"] / df["DSAPC"]
            df["MTSI"] = df["TSI"].ewm(span=signal_ma, adjust=False).mean()
            df["Signal"] = df["MTSI"].rolling(window=signal_ma).mean()
            df["Buy"] = (df["MTSI"] > df["Signal"]) & (
                df["MTSI"].shift(1) <= df["Signal"].shift(1)
            )
            df["Sell"] = (df["MTSI"] < df["Signal"]) & (
                df["MTSI"].shift(1) >= df["Signal"].shift(1)
            )

            print(f"self.coconut_df.iloc[-1] {self.coconut_df.iloc[-1]}")

        return self.coconut_df.iloc[-1]
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], Any, Any]:
        ...
        # COCONUT STRAT
        try:
            result[COCONUT] = self.coconut_mtsi_strategy(state)
        except Exception as e:
