...
class Trader:
    ...
    def coconut_predictive_strategy(self, state: TradingState):
        ...
        coupon_buy_quantity = (
            self.position_limit[COCONUT_COUPON] - coupon_position_coconut
        )
        coupon_sell_quantity = (
            -self.position_limit[COCONUT_COUPON] - coupon_position_coconut
        )
        steps = 5
        model_order = 3
        ...
        if len(self.coconut_price) > 400:
            ...
            if y_hat_test[0] > coconut_price:
                coconut_orders.append(
                    Order(COCONUT, int(coconut_price - 1), buy_quantity)
                )  # buying coconut
                coconut_coupon.append(
                    Order(
                        COCONUT_COUPON,
                        int(coconut_coupon_price + 1),
                        coupon_sell_quantity,
                    )
                )  # sell a call option
            else:
                coconut_orders.append(
                    Order(COCONUT, int(coconut_price + 1), sell_quantity)
                )
                coconut_coupon.append(
                    Order(
                        COCONUT_COUPON,
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], Any, Any]:
        ...
        try:
            result[COCONUT], result[COCONUT_COUPON] = self.coconut_predictive_strategy(
                state
            )
        except Exception as e:
