...
#strat for startfruit (finalized)
    ...
    def Orchid_strategy(self, state: TradingState):
        ...
        if best_ask and best_bid:
            ...
            if buy_price_from_south_archipelago < mid_price:
                orders.append(Order(ORCHIDS,  int(mid_price-1), sell_quantity))
                conversion_amt = -sell_quantity
            if sell_price_to_south_archipelago > mid_price:
                orders.append(Order(ORCHIDS,  int(mid_price + 1), buy_quantity))
                conversion_amt = -buy_quantity
            print(buy_price_from_south_archipelago,sell_price_to_south_archipelago,mid_price)
    ...
    def coconut_predictive_strategy(self, state: TradingState):
        ...
        if len(self.coconut_price) > 500: # initial: 350
            ...
            if y_hat_test[1] >y_hat_test[0]> coconut_price:
                coconut_orders.append(Order(COCONUT, int(coconut_price), buy_quantity)) # buying coconut 
                coconut_coupon.append(Order(COCONUT_COUPON, int(coconut_coupon_price), coupon_sell_quantity)) # sell a call option 
            else:
                coconut_orders.append(Order(COCONUT, int(coconut_price), sell_quantity))
                coconut_coupon.append(Order(COCONUT_COUPON, int(coconut_coupon_price), buy_quantity))
               
        return coconut_orders, coconut_coupon
    
    def orchids_predictive_strategy(self, state: TradingState):
        orders = []
        forecast_steps = 5
        orchids_price = self.get_mid_price(ORCHIDS, state)
        self.orchid_prices.append(orchids_price)
        #x_data = state.timestamp
        #y_data = self.coconut_price
        self.timestamp.append(state.timestamp)
        position = self.get_position(ORCHIDS, state)
        buy_quantity = self.position_limit[ORCHIDS] - position
        sell_quantity = -self.position_limit[ORCHIDS] - position

        steps = 5
        model_order = 3
        model_reg = 0.001
        model = AdaptiveTrendFiltering(order=1, regularization=model_reg)
        if len(self.orchid_prices) > 200: # initial: 350
            x_train, x_test = self.timestamp[:-steps], self.timestamp[-steps:]
            y_train, y_test = self.orchid_prices[:-steps], self.orchid_prices[-steps:]
            # Fit the model to the data and get the estimated y_hat
            y_hat_train = model.fit(x_train, y_train)
            y_hat_test = model.forecast(x_test)
            if  y_hat_test[0]> orchids_price:
                orders.append(Order(COCONUT, int(orchids_price), buy_quantity)) 
            else:
                orders.append(Order(COCONUT, int(orchids_price), sell_quantity))
        return orders
      ...
      try:
        ...
        if not np.isnan(avg_spread.iloc[-1]):
          ...
          if spread_5 < avg_spread - 1.5*std_spread:       
            ...
            orders_strawberries.append(Order('STRAWBERRIES', int(price_strawberries), 18)) #18, 6
            orders_roses.append(Order('ROSES', int(price_roses), 3)) # 3, 1
            orders_choco.append(Order('CHOCOLATE', int(price_chocolate), -12)) #12
            ...
            # Buy GIFT_BASKET and sell components
              ...
              orders_strawberries.append(Order('STRAWBERRIES', int(price_strawberries), -18))
              orders_roses.append(Order('ROSES', int(price_roses), -3))
              orders_choco.append(Order('CHOCOLATE', int(price_chocolate), 12))
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], Any, Any]:
        ...
        # print("+---------------------------------+")
        # try:
        #     result[ORCHIDS], conv = self.Orchid_strategy(state)
        #     self.conversion_amt = conv
