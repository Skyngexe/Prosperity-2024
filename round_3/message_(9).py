...
class Trader:
    ...
    def gb_strategy(
        ...
        if len(self.chocolate_past_price) > 2:
            predicted_basket_price = (
                82.167201  # Constant for GIFT_BASKET
                + 0.0409 * self.chocolate_past_price[-1]  # L1 coefficient for CHOCOLATE
                + 0.0057 * self.chocolate_past_price[-2]  # L2 coefficient for CHOCOLATE
                - 0.0393 * self.basket_past_price[-1]  # L1 coefficient for GIFT_BASKET
                - 0.0028 * self.basket_past_price[-2]  # L2 coefficient for GIFT_BASKET
                + 0.0239 * self.rose_past_price[-1]  # L1 coefficient for ROSES
                - 0.0115 * self.rose_past_price[-2]  # L2 coefficient for ROSES
                + 0.0078
                * self.strawberries_past_price[-1]  # L1 coefficient for STRAWBERRIES
                + 0.0256
                * self.strawberries_past_price[-2]  # L2 coefficient for STRAWBERRIES
            )

            adjustment_factor = price_basket - predicted_basket_price

            if len(self.adjustment_factor_array) < 20:
                self.adjustment_factor_array.append(adjustment_factor)
            else:
                self.adjustment_factor_array.pop(0)
                self.adjustment_factor_array.append(adjustment_factor)

            avg_adjustment_factor = np.mean(self.adjustment_factor_array)
            actual_predicted_basket_price = (
                predicted_basket_price + avg_adjustment_factor
            )
            print(
                "actual_predicted_basket_price: ",
                actual_predicted_basket_price,
        ...
        if actual_predicted_basket_price > price_basket:
            orders_basket.append(
                Order(GIFT_BASKET, int(price_basket), VOLUME_BASKET)
            )  # buy
    ...
    # finalized
    def benny_gift_basket_strategy(self, state: TradingState) -> List[Order]:
        orders_basket = []
        orders_roses = []
        orders_choco = []
        orders_strawberries = []
            ...
            if not np.isnan(avg_spread.iloc[-1]):
                ...
                if z_score > z_score_threshold:
                    ...
                    orders_strawberries.append(
                        Order("STRAWBERRIES", int(price_strawberries), -18)
                    )
                    ...
                    orders_roses.append(Order("ROSES", int(price_roses), 3))  # 3
                    orders_basket.append(
                        Order("CHOCOLATE", int(price_chocolate), 12)
                    )  # 12
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], Any, Any]:
        ...
        try:
            (
                ...
                result[STRAWBERRIES],
                result[ROSES],
            ) = self.benny_gift_basket_strategy(state)
        except Exception as e:
            print("Error in gift basket strategy")
            print(e)
        # try:
