...
class Trader:
    ...
    def amethysts_strategy(self, state: TradingState):
      ...
      if mid_price > 9999:
        orders.append(Order(AMETHYSTS, int(mid_price+2), ask_volume)) # short it 
      else:
        orders.append(Order(AMETHYSTS, int(mid_price-2), bid_volume)) # buy it
      return orders
...
#strat for startfruit
    ...
    def Orchid_strategy(self, state: TradingState):
        ...
        if best_ask and best_bid:
            ...
            sell_to_south_archipelago = average_log_return * mid_price *(optimal_quantity + sell_quantity) + (-sell_quantity*south_archipelago_ask_price) -(0.1*(optimal_quantity+ sell_quantity))
            ...
            decision = max(sell_on_our_archipelago, sell_to_south_archipelago, buy, fuck_it_we_ball)
